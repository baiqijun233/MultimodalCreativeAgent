...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; } body { margin:0; background:var(--bg); color:var(--text); } .shell { max-width:1280px; margin:0 auto; padding:28px 24px 48px; }
    ...
    @media (max-width: 820px) { .shell { padding:20px 14px 32px; } header { align-items:flex-start; flex-direction:column; } main { grid-template-columns:1fr; } table { min-width:0; } th, td { padding:10px 8px; font-size:12px; } td.request { max-width:none; white-space:normal; } }
    @media (max-width: 560px) { table th:nth-child(3), table td:nth-child(3), table th:nth-child(4), table td:nth-child(4) { display:none; } table th:nth-child(1), table td:nth-child(1) { width:76px; } table th:nth-child(5), table td:nth-child(5) { width:88px; } }
  </style>
</head>
<body>
  <div class="shell">
